...
import { getEsServiceBySlug, estoEnServiceSlug } from '../../data/es-services-data';
import SEO from '../SEO';
import { trackServiceView, trackPhoneClick, trackCtaClick } from '../../utils/analytics';

export default function SpanishServiceDetailPage() {
  const { area } = useParams<{ area: string }>();
  const navigate = useNavigate();
  const service = area ? getEsServiceBySlug(area) : undefined;

  useEffect(() => {
    if (service) {
      trackServiceView({ service_name: service.title, service_slug: service.slug, language: 'es' });
    }
  }, [service?.slug]);
  ...
  if (!service) {
    return (
      <div className="section-padding text-center">
        <h1 className="text-3xl font-serif font-bold text-[#1B2A4A] mb-4">Página No Encontrada</h1>
        <p className="text-gray-500 mb-8">Ese servicio no existe.</p>
        <Link to="/es/servicios" className="btn-gold">Volver a Servicios</Link>
      </div>
    );
  }
  ...
  return (
    <div>
      <SEO
        title={`${service.title} | Abogados de Inmigración Los Ángeles`}
        description={service.metaDescription}
        canonical={`/es/servicios/${service.slug}`}
        ogImage={`https://www.makeitglow.co/api/og/becraft-immigration?title=${encodeURIComponent(service.title)}&subtitle=${encodeURIComponent('Abogados de Inmigración · Los Ángeles')}`}
        schema={{ "@context": "https://schema.org", "@type": "LegalService", "name": "Law Office of Todd Becraft", "url": "https://becraftimmigrationlaw.com" }}
        hreflang={estoEnServiceSlug[service.slug] ? [
          { lang: 'en', path: `/services/${estoEnServiceSlug[service.slug]}` },
          { lang: 'es', path: `/es/servicios/${service.slug}` },
          { lang: 'x-default', path: `/services/${estoEnServiceSlug[service.slug]}` },
        ] : undefined}
      />
      {/* Hero */}
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-28">
          <div className="max-w-3xl">
            <nav className="mb-4 text-sm text-gray-400 flex items-center gap-2">
              <Link to="/es/servicios" className="hover:text-[#D4A853] transition-colors">
                Servicios
              </Link>
              <span>/</span>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-[#1B2A4A] rounded-sm p-8 text-center">
            <h2 className="text-2xl font-serif text-white font-bold mb-3">
              ¿Necesita Ayuda con {service.title}?
            </h2>
            <p className="text-gray-300 leading-relaxed mb-6">
              Nuestros abogados de inmigración están listos para evaluar su caso. Consulta gratis disponible.
            </p>
            <div className="flex flex-col sm:flex-row justify-center gap-4">
              <a href="tel:2133881821" onClick={() => trackPhoneClick({ phone_number: '2133881821', location: 'service_cta', language: 'es' })} className="btn-gold">
                Llame al (213) 388-1821
              </a>
              <button onClick={() => { trackCtaClick({ label: 'contact_us_online', location: 'service_cta', language: 'es' }); navigate('/es/contactenos'); }} className="btn-outline">
                Envíenos un Mensaje
              </button>
            </div>
          </div>
