...
import { getEsResourceBySlug } from '../../data/es-resources-data';
import SEO from '../SEO';
import { trackResourceView } from '../../utils/analytics';

export default function SpanishResourcePage() {
  const { slug } = useParams<{ slug: string }>();
  const navigate = useNavigate();
  const resource = slug ? getEsResourceBySlug(slug) : undefined;

  useEffect(() => {
    if (resource) {
      trackResourceView({ resource_name: resource.title, resource_slug: resource.slug, language: 'es' });
    }
  }, [resource?.slug]);
  ...
  if (!resource) {
    return (
      <div className="section-padding text-center">
        ...
        <p className="text-gray-500 mb-8">Ese recurso no existe.</p>
        <Link to="/es/recursos" className="btn-gold">Volver a Recursos</Link>
      </div>
    );
  }
  ...
  return (
    <div>
      <SEO
        title={`${resource.title} | Abogados de Inmigración Los Ángeles`}
        description={resource.metaDescription}
        canonical={`/es/recursos/${resource.slug}`}
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-28">
          <div className="max-w-3xl">
            <nav className="mb-4 text-sm text-gray-400 flex items-center gap-2">
              <Link to="/es/recursos" className="hover:text-[#D4A853] transition-colors">
                Recursos
              </Link>
              <span>/</span>
